...
import ordersFakerFactory from "./ordersFakerFactory";
...
const OrdersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = ordersFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("orders").create(elem));
...
export default connect(null, mapDispatch)(OrdersFakerDialogComponent);
